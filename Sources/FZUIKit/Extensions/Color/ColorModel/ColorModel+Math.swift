...
enum ColorMathVector {
    ...
    @inline(__always)
    private static func hueToVector(_ h: Double) -> (x: Double, y: Double) {
        let θ = h * 2 * .pi
        return (cos(θ), sin(θ))
    }
    
    @inline(__always)
    private static func hueFromVector(_ x: Double, _ y: Double) -> Double {
        let hue = atan2(y, x) / (2 * .pi)
        return hue < 0 ? hue + 1 : hue
        // return atan2(y, x) / (2 * .pi)
    }
...
extension ColorMathVector {
    static func colorToRGB(_ color: SIMD4<Double>, colorspace: ColorModels.ColorSpace) -> SIMD4<Double> {
        switch colorspace {
    ...
    static func rgbToColorSpace(_ color: SIMD4<Double>, colorspace: ColorModels.ColorSpace) -> SIMD4<Double> {
        switch colorspace {
        case .srgb: return color
    ...
    static func rgbToAnimatable(_ color: SIMD4<Double>, colorspace: ColorModels.ColorSpace) -> AnimatablePair<SIMD4<Double>, Double> {
        var color = rgbToColorSpace(color, colorspace: colorspace)
        switch colorspace {
        case .hpluv, .hsb, .hsl, .hsluv, .hwb, .okhsb, .okhsl:
            let vector = hueToVector(color[0])
            color[0] = vector.x
            return AnimatablePair(color, vector.y)
        case .jzczhz, .lch, .lchuv, .oklch:
            let vector = hueToVector(color[2])
            color[2] = vector.x
            return AnimatablePair(color, vector.y)
        default:
            return AnimatablePair(color, 0)
        }
    }
    
    static func animatableToRGB(_ animatable: AnimatablePair<SIMD4<Double>, Double>, colorspace: ColorModels.ColorSpace) -> SIMD4<Double> {
        switch colorspace {
        case .hpluv, .hsb, .hsl, .hsluv, .hwb, .okhsb, .okhsl:
