...
public extension CGColorSpace {
    ...
    struct Name: Sendable {
        ...
        public init(rawValue: String, model: CGColorSpaceModel) {
            self.rawValue = rawValue
            self.model = model
        }
        
        public init(rawValue: CFString, model: CGColorSpaceModel) {
            self.init(rawValue: rawValue as String, model: model)
        }
        
        private init(_ rawValue: String, model: CGColorSpaceModel = .rgb) {
            self.init(rawValue: rawValue, model: model)
        }
        
        private init(_ rawValue: CFString, model: CGColorSpaceModel = .rgb) {
            self.init(rawValue: rawValue as String, model: model)
        }
        ...
        public static let deviceGray = Self("kCGColorSpaceDeviceGray", model: .monochrome)
        ...
        public static let genericGray = Self("kCGColorSpaceGenericGray", model: .monochrome)
        ...
        public static let deviceCMYK = Self("kCGColorSpaceDeviceCMYK", model: .cmyk)
    ...
    static func availableColorSpaces() -> [CGColorSpace] {
        Name.availableNames.compactMap({ $0.colorSpace })
    }
    ...
    static func availableColorSpaces(with model: CGColorSpaceModel) -> [CGColorSpace] {
        Name.availableNames(with: model).compactMap({ $0.colorSpace })
    }
}
