...
public extension NSEvent {
    ...
    static func activateLocalMonitors(for mask: NSEvent.EventTypeMask? = nil) {
        if let mask = mask {
    ...
    static func deactivateLocalMonitors(for mask: NSEvent.EventTypeMask? = nil) {
        if let mask = mask {
    ...
    static func activateGlobalMonitors(for mask: NSEvent.EventTypeMask? = nil) {
        if let mask = mask {
    ...
    static func deactivateGlobalMonitors(for mask: NSEvent.EventTypeMask? = nil) {
        if let mask = mask {
    ...
    internal static var monitors: [UUID: Weak<NSEvent.Monitor>] = [:]
    
    internal static var localMonitors: [Monitor] {
        monitors.values.compactMap({$0.object}).filter({$0.type == .local})
    }
    
    internal static var globalMonitors: [Monitor] {
    ...
    class Monitor {
        ...
        private init(mask: NSEvent.EventTypeMask, type: MonitorType = .local, handler: Any) {
            ...
            self.type = type
            start()
            NSEvent.monitors[id] = .init(self)
        }
        
        deinit {
            NSEvent.monitors[id] = nil
            stop()
        }
