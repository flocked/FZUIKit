...
//
    ...
    open class Toolbar: NSObject {
        ...
        static func checkedItems(_ items: [ToolbarItem]) -> [ToolbarItem] {
            ...
            for item in items {
                if let toolbar = item.toolbar, toolbar !== self {
    ...
    public extension NSPopover {
        /**
         ...
         Use this method to display a popover relative to a toolbar item. When the item is in the overflow menu, the popover presents itself from another appropriate affordance in the window. See [show(relativeTo:of:preferredEdge:)](https://developer.apple.com/documentation/appkit/nspopover/show(relativeto:of:preferrededge:)) for popover behavior.
