...
public extension NSResponder {
    ...
    /**
     Prints the reponder chain.
     
     - Parameter includeDetails: If `true` prints the full description of each responder; otherwise prints only the type.
     */
    func printResponderChain(includeDetails: Bool = false) {
        for (i, responder) in responderChain().indexed() {
            let string = includeDetails ? "\(responder)" : "\(type(of: responder))"
            Swift.print("\(Array(repeating: " ", count: i).joined(separator: ""))\(string)")
        }
    }
