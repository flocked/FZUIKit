...
public class ViewAnimator {
    ...
    public var backgroundColor: NSUIColor? {
        ...
        set {
            setValue(newValue, \.backgroundColor, \.backgroundColor)
            /*
    ...
    internal var shadowRadius: CGFloat {
        get {
            value(for: \.shadowRadius)
           // animation(for: \.shadowRadius)?.target ?? view.shadowRadius
        }
        set {
            setValue(newValue, for: \.shadowRadius)
...
extension ViewAnimator {
    ...
    private func animationNew<Val>(for keyPath: WritableKeyPath<NSView, Val?>) -> SpringAnimator<Val>? {
        guard let keyPath = keyPath._kvcKeyPathString else { return nil }
        return view.waveAnimationsNew[keyPath] as? SpringAnimator<Val>
    }
...
public extension NSUIView {
    ...
    internal var waveAnimationsNew: [String: AnimationProviding] {
        get { getAssociatedValue(key: "waveAnimations", object: self, initialValue: [:]) }
        set { set(associatedValue: newValue, key: "waveAnimations", object: self) }
    }
}

internal extension ViewAnimator {
    func value<Value: SpringInterpolatable>(for keyPath: WritableKeyPath<NSView, Value>) -> Value where Value.ValueType == Value, Value.VelocityType == Value {
        return animationNew(for: keyPath)?.target ?? view[keyPath: keyPath]
    }
    ...
    func setValue<Value: SpringInterpolatable>(_ newValue: Value, for keyPath: WritableKeyPath<NSView, Value>) where Value.ValueType == Value, Value.VelocityType == Value {
        guard animationNew(for: keyPath)?.target ?? view[keyPath: keyPath] != newValue else {
            return
        }
        ...
        AnimationController.shared.executeHandler(uuid: animationNew(for: keyPath)?.groupUUID, finished: false, retargeted: true)

        let animation = (animationNew(for: keyPath) ?? SpringAnimator<Value>(spring: settings.spring, value: initialValue, target: targetValue))
        animation.configure(withSettings: settings)
        if let gestureVelocity = settings.gestureVelocity {
        ...
        animation.completion = { [weak self] event in
            switch event {
            case .finished:
                self?.view.waveAnimationsNew.removeValue(forKey: animationKey)
                AnimationController.shared.executeHandler(uuid: groupUUID, finished: true, retargeted: false)
            default:
                break
            }
        }
        view.waveAnimationsNew[animationKey] = animation
...
extension Animator where Object: NSView {
    var frame: CGRect {
        get { getValue(\.frame, \.frame) }
        set { setValue(newValue, \.frame, \.frame) }
    }
}

class Animator<Object: Animatable> {
    private var object: Object
    internal init(_ object: Object) {
        self.object = object
    }
    
    private func animation<Value>(for keyPath: WritableKeyPath<Animator, Value?>) -> SpringAnimator<Value>? {
        object.waveAnimationsNew[keyPath] as? SpringAnimator<Value>
    }
    ...
    private func getValue<Value: SpringInterpolatable>(_ keyPath: WritableKeyPath<Animator, Value>, _ viewKeyPath: WritableKeyPath<Object, Value>) -> Value where Value.ValueType == Value, Value.VelocityType == Value {
        animation(for: keyPath)?.target ?? object[keyPath: viewKeyPath]
    }
    ...
    private func setValue<Value: SpringInterpolatable>(_ targetValue: Value, _ keyPath: WritableKeyPath<Animator, Value>, _ viewKeyPath: WritableKeyPath<Object, Value>) where Value.ValueType == Value, Value.VelocityType == Value {
        guard self[keyPath: keyPath] != targetValue else {
            return
        }
        
        guard let settings = AnimationController.shared.currentAnimationParameters else {
            Wave.animate(withSpring: .nonAnimated, mode: .nonAnimated) {
                self.object.animatorNew[keyPath: keyPath] = targetValue
            }
            return
        }
        
        let initialValue = object[keyPath: viewKeyPath]
        ...
        animation.target = targetValue
        animation.valueChanged = { [weak self] value in
            self?.object[keyPath: viewKeyPath] = value
        }
        let groupUUID = animation.groupUUID
        animation.completion = { [weak self] event in
            switch event {
            case .finished:
                self?.object.waveAnimationsNew.removeValue(forKey: keyPath)
                AnimationController.shared.executeHandler(uuid: groupUUID, finished: true, retargeted: false)
            default:
                break
            }
        }
        object.waveAnimationsNew[keyPath] = animation
        animation.start(afterDelay: settings.delay)
    }
    
    func setValue<Value: SpringInterpolatable>(_ newValue: Value?, _ keyPath: WritableKeyPath<Animator, Value?>, _ viewKeyPath: WritableKeyPath<Object, Value?>) where Value.ValueType == Value, Value.VelocityType == Value {
        ...
        guard let settings = AnimationController.shared.currentAnimationParameters else {
            Wave.animate(withSpring: .nonAnimated, mode: .nonAnimated) {
                self.object.animatorNew[keyPath: keyPath] = newValue
            }
            return
        }
        
        let initialValue = object[keyPath: viewKeyPath] ?? Value.VelocityType.zero
        let targetValue = newValue ?? Value.VelocityType.zero
...
protocol Animatable: NSObject {
    associatedtype Ani
    var animatorNew: Ani { get set }
    var waveAnimationsNew: [PartialKeyPath<Ani>: AnimationProviding] { get set }
}

extension Animatable {
    var animatorNew: Animator<Self> {
        get { getAssociatedValue(key: "animator", object: self, initialValue: Animator(self)) }
        set { set(associatedValue: newValue, key: "animator", object: self) }
    }
    var waveAnimationsNew: [PartialKeyPath<Animator<Self>>: AnimationProviding] {
        get { getAssociatedValue(key: "waveAnimations", object: self, initialValue: [:]) }
        set { set(associatedValue: newValue, key: "waveAnimations", object: self) }
    }
}
