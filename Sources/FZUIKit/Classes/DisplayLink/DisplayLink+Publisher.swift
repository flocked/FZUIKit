...
protocol DisplayLinkProvider: AnyObject {
    ...
    var onFrame: ((DisplayLinkPublisher.Frame) -> Void)? { get set }
}

/// A publisher that emits new values when the system is about to update the display.
public final class DisplayLinkPublisher: Publisher {
...
public extension DisplayLinkPublisher {
    static let shared = DisplayLinkPublisher()
}

private extension DisplayLinkPublisher {
...
fileprivate extension DisplayLinkPublisher {
    final class PlatformDisplayLink: DisplayLinkProvider {
        ...
        final class DisplayLinkTarget {
            var callback: ((DisplayLinkPublisher.Frame) -> Void)?
    ...
    final class PlatformDisplayLink: DisplayLinkProvider {
        ...
        init() {
            CVDisplayLinkSetOutputHandler(displayLink) { [weak self] _, inNow, inOutputTime, _, _ -> CVReturn in
                let frame = Frame(timestamp: inNow.pointee.timeInterval, duration: (Double(inOutputTime.pointee.timeInterval) - Double(inNow.pointee.timeInterval)) / 2.0)
