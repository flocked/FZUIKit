...
//
    ...
    public extension NSView {
        /**
         var     Observes the view by adding a hidden ``ObserverView`` as subview to the receiver and returning it.
              ...
              */
        func observe() -> ObserverView {
            if let observingView = subviews(type: ObserverView.self).first {
                return observingView
            }
            let observingView = ObserverView(frame: .zero)
            addSubview(withAutoresizing: observingView)
    ...
    public class ObserverView: NSView {
        ...
        public var viewHandlers = ViewHandlers() {
            didSet {
                setupObservation(needsSetup: viewHandlers.needsSetup)
            }
        }
        ...
        override public func mouseEntered(with event: NSEvent) {
            if mouseHandlers.entered?(event) ?? true {
                super.mouseEntered(with: event)
            }
        }

        override public func mouseExited(with event: NSEvent) {
            if mouseHandlers.exited?(event) ?? true {
                super.mouseExited(with: event)
            }
        }

        override public func mouseDown(with event: NSEvent) {
            if mouseHandlers.down?(event) ?? true {
                super.mouseDown(with: event)
            }
        }

        override public func rightMouseDown(with event: NSEvent) {
            if mouseHandlers.rightDown?(event) ?? true {
                super.rightMouseDown(with: event)
            }
        }

        override public func mouseUp(with event: NSEvent) {
            if mouseHandlers.up?(event) ?? true {
                super.mouseUp(with: event)
            }
        }

        override public func rightMouseUp(with event: NSEvent) {
            if mouseHandlers.rightUp?(event) ?? true {
                super.rightMouseUp(with: event)
            }
        }

        override public func mouseMoved(with event: NSEvent) {
            if mouseHandlers.moved?(event) ?? true {
                super.mouseMoved(with: event)
            }
        }

        override public func mouseDragged(with event: NSEvent) {
            if mouseHandlers.dragged?(event) ?? true {
        ...
        override public func viewDidMoveToWindow() {
            if let window = window {
                windowHandlers.didMoveToWindow?(window)
            }
        ...
        override public func viewWillMove(toWindow newWindow: NSWindow?) {
            ...
            windowHandlers.willMoveToWindow?(newWindow)
        ...
        var windowIsKey = false {
            didSet {
                if oldValue != windowIsKey {
                    windowHandlers.isKey?(windowIsKey)
                }
            }
        }
        ...
        var windowIsMain = false {
            didSet {
                if oldValue != windowIsMain {
                    windowHandlers.isMain?(windowIsMain)
                }
            }
        }
    ...
    public extension NSView {
        var windowHandlersNew: WindowHandlers {
        ...
        var mouseHandlersNew: MouseHandlers {
