...
//
    ...
    public class QuicklookPanel: NSResponder {
        ...
        internal var items = [QLPreviewable]() {
            didSet {
                let oldPreviewableMedia = oldValue.compactMap { $0 as? QLPreviewableMedia }
                deleteTemporaryMediaFiles(for: oldPreviewableMedia)
            }
        }

        internal func deleteTemporaryMediaFiles(for mediaItems: [QLPreviewableMedia]) {
            let newItemURLs = items.compactMap { ($0 as? QLPreviewableMedia)?.previewItemURL }
            let mediaItems = mediaItems.filter { newItemURLs.contains($0.previewItemURL) == false }
            mediaItems.forEach { try? $0.deleteTemporaryPreviewFile() }
        }
        ...
        public func present(url: URL, frame: CGRect? = nil) {
            present([QuicklookItem(url: url, frame: frame)])
        }

        public func present(urls: [URL], frame: CGRect? = nil, currentItemIndex: Int = 0) {
            present(urls.compactMap { QuicklookItem(url: $0, frame: frame) }, currentItemIndex: currentItemIndex)
        }
    ...
    extension QuicklookPanel: QLPreviewPanelDataSource, QLPreviewPanelDelegate {
        ...
        public func previewPanel(_: QLPreviewPanel!, sourceFrameOnScreenFor item: QLPreviewItem!) -> NSRect {
            if let itemView = (item as? QLPreviewable)?._itemView {
                if itemView.isVisible, let frameOnScreen = itemView.frameOnScreen {
            ...
            if let itemsProviderWindow = itemsProviderWindow {
                if let itemFrame = (item as? QLPreviewable)?.frameOnScreen(inside: itemsProviderWindow) {
                    return itemFrame
                } else {
