...
public extension AnimatableProperty where Self.AnimatableData: Comparable {
...
public extension AnimatableProperty where Self: NSUIColor {
    init(_ animatableData: AnimatableArray<Double>) {
        #if os(macOS)
...
extension CGColor: AnimatableProperty {
    public var animatableData: AnimatableArray<Double> {
        let components = rgbaComponents() ?? .zero
...
extension CGVector: AnimatableProperty, Animatable {
    ...
    extension CGQuaternion: AnimatableProperty, Animatable {
        public init(_ animatableData: AnimatableArray<Double>) {
            storage = .init(ix: animatableData[0], iy: animatableData[1], iz: animatableData[2], r: animatableData[3])
        }

        public var animatableData: AnimatableArray<Double> {
            get { [storage.vector[0], storage.vector[1], storage.vector[2], storage.vector[3]] }
            set { self = .init(newValue) }
        }

        public static var zero: CGQuaternion {
            CGQuaternion(degree: 0, axis: .init(0, 0, 0))
        }
    }
    ...
    extension CGVector3: AnimatableProperty, Animatable, Hashable {
        ...
        public init(_ animatableData: AnimatableArray<Double>) {
            storage = .init(x: animatableData[0], y: animatableData[1], z: animatableData[2])
        }
    ...
    extension CGVector4: AnimatableProperty, Animatable {
        public init(_ animatableData: AnimatableArray<Double>) {
            storage = .init(x: animatableData[0], y: animatableData[1], z: animatableData[2], w: animatableData[3])
        }
    ...
    extension ShadowConfiguration: AnimatableProperty, Animatable {
        ...
        public var animatableData: AnimatableArray<Double> {
            get { (_resolvedColor ?? .zero).animatableData + [opacity, radius, offset.x, offset.y] }
            set { self = .init(newValue) }
        }
    }
    ...
    extension BorderConfiguration: AnimatableProperty, Animatable {
        ...
        public var animatableData: AnimatableArray<Double> {
            get { (_resolvedColor ?? .zero).animatableData + [width] }
            set { self = .init(newValue) }
        }
    }
#endif
