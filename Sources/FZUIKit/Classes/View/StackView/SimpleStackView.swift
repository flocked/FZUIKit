...
open class SimpleStackView: NSUIView {
    ...
    open func setDistribution(_ distribution: ViewDistribution, for arrangedSubview: NSUIView) {
        ...
        guard viewDistributions[arrangedSubview.objectID] != distribution else { return }
        viewDistributions[arrangedSubview.objectID] = distribution
        updateViewConstraints()
    }
    ...
    var viewObservers: [ObjectIdentifier: KeyValueObservation] = [:]
    var viewDistributions: [ObjectIdentifier: ViewDistribution] = [:]
    ...
    func setupManagedViews(previous: [NSUIView] = []) {
        ...
        diff.removed.forEach {
            ...
            viewDistributions[$0.objectID] = nil
        }

        diff.added.forEach {
            $0.translatesAutoresizingMaskIntoConstraints = false
            addObserver(for: $0)
            viewDistributions[$0.objectID] = .fill
            addSubview($0)
        }
    ...
    func addObserver(for view: NSUIView) {
        viewObservers[view.objectID] = view.observeChanges(for: \.isHidden, handler: { [weak self] old, new in
    ...
    func removeObserver(for view: NSUIView) {
        viewObservers[view.objectID] = nil
    }
    ...
    func updateViewConstraints() {
        ...
        for (index, managedView) in nonHiddenViews.enumerated() {
            let distribution = viewDistributions[managedView.objectID] ?? .fill
